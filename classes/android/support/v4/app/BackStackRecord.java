...
final class BackStackRecord extends FragmentTransaction implements FragmentManager$BackStackEntry, FragmentManagerImpl$OpGenerator {
  ...
  ArrayList<BackStackRecord$Op> mOps = new ArrayList<BackStackRecord$Op>();
  ...
  private void doAddOp(int paramInt1, Fragment paramFragment, String paramString, int paramInt2) {
    ...
    backStackRecord$Op.cmd = paramInt2;
    backStackRecord$Op.fragment = paramFragment;
    addOp(backStackRecord$Op);
  }
  
  private static boolean isFragmentPostponed(BackStackRecord$Op paramBackStackRecord$Op) {
  ...
  void addOp(BackStackRecord$Op paramBackStackRecord$Op) {
          ...
          if (k >= 0) {
            ...
            if (fragment1.mContainerId == m) {
              if (fragment1 == fragment) {
                ...
                BackStackRecord$Op backStackRecord$Op1 = new BackStackRecord$Op();
                backStackRecord$Op1.cmd = 3;
                backStackRecord$Op1.fragment = fragment1;
                ...
                this.mOps.add(i, backStackRecord$Op1);
  ...
  boolean interactsWith(int paramInt) {
    int j = this.mOps.size();
    for (int i = 0; i < j; i++) {
      if (((BackStackRecord$Op)this.mOps.get(i)).fragment.mContainerId == paramInt)
        return true; 
    } 
    return false;
  }
  ...
  boolean interactsWith(ArrayList<BackStackRecord> paramArrayList, int paramInt1, int paramInt2) {
    ...
    int j;
    for (j = 0; j < k; j++) {
      int m = ((BackStackRecord$Op)this.mOps.get(j)).fragment.mContainerId;
      if (m != 0 && m != i) {
        for (i = paramInt1; i < paramInt2; i++) {
          ...
          int n;
          for (n = 0; n < i1; n++) {
            if (((BackStackRecord$Op)backStackRecord.mOps.get(n)).fragment.mContainerId == m)
              return true; 
          } 
        } 
        i = m;
  ...
  void setOnStartPostponedListener(Fragment$OnStartEnterTransitionListener paramFragment$OnStartEnterTransitionListener) {
...
/* Location:              C:\Users\devid\Downloads\SAST\Santander\dex2jar-2.0\classes-dex2jar.jar!\android\support\v4\app\BackStackRecord.class
