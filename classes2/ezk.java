final class ezk {
  private static final int[][] a = new int[][] { { 27, 917 }, { 522, 568, 723, 809 }, { 237, 308, 436, 284, 646, 653, 428, 379 }, { 
  ...
  static String a(CharSequence paramCharSequence, int paramInt) {
    ...
    for (int i = 0; i < k; i++) {
      ...
      for (int m = j - 1; m >= 1; m--)
        arrayOfChar[m] = (char)((929 - a[paramInt][m] * n % 929 + arrayOfChar[m - 1]) % 929); 
      arrayOfChar[0] = (char)((929 - a[paramInt][0] * n % 929) % 929);
    } 
    paramCharSequence = new StringBuilder(j);
    for (paramInt = j - 1; paramInt >= 0; paramInt--) {
      if (arrayOfChar[paramInt] != '\000')
        arrayOfChar[paramInt] = (char)(929 - arrayOfChar[paramInt]); 
  ...
  static int b(int paramInt) {
    ...
    if (paramInt <= 863)
      return 5; 
    throw new eqr("No recommendation possible");
  }
}


/* Location:              C:\Users\devid\Downloads\SAST\Santander\dex2jar-2.0\classes2-dex2jar.jar!\ezk.class
